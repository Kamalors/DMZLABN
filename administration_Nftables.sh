...
function add_predefined_rules() {
    ...
    case $choice in
        1)
	    list_rules
	    echo -n "Où veux-tu placer la règle ? (la règle sera placé après le nombre indiqué)"
	    read position
            nft add rule inet filter input position $position drop
	    ...
	    echo -n "Où veux-tu placer la règle ? (la règle sera placé après le nombre indiqué)"
	    read position
            nft add rule inet filter input position $position ip saddr $ip  tcp dport 22 accept
            echo "Règle SSH ajoutée pour l'IP $ip à la position $position."
            ;;
        3)
...
function delete_rule() {
    echo "Entrez la table et la chaine contenant la règle à supprimer (ex : '(nom de la table exemple : inet filter) (nom de la chaine : input)') :"
    read chain
...
function restore_config() {
	...
	cat $BACKUP_DIR/$backup_file > $NFT_CONFIG
